...
ggsurvplot(lapse_basic_km, censor = FALSE, break.time.by = 12)
```
...
ggsurvplot(lapse_gender_km, censor = FALSE, break.time.by = 12)
```
...
ggsurvplot(lapse_smoker_km, censor = FALSE, break.time.by = 12)
```
...
basic_01_model <- coxph(Surv(month_reached, lapsed) ~ gender_life1
                       ,data = split_lst$train
                        )

basic_01_model %>% summary
basic_01_model %>% tidy()
basic_01_model %>% glance()
```
...
We also want to add smoking status, so we try that.

```{r fit_02_model, echo=TRUE}
basic_02_model <- coxph(Surv(month_reached, lapsed) ~ gender_life1 + smoker_life1
                       ,data = split_lst$train
                        )

basic_02_model %>% summary
basic_02_model %>% tidy()
basic_02_model %>% glance()
```
